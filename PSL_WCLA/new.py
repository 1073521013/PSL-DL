...
#from cnn2 import CNN_LSTM   #OPTION 0
...
#            model = LSTM(x_train.shape[1],y_train.shape[1],len(vocab_processor.vocabulary_),embedding_dim)
        ...
        checkpoint_dir = os.path.abspath(os.path.join("runs2", "checkpoints"))
        ...
        def dev_step(x_batch, y_batch,writer=None,save=True):
            ...
            step, predictions,summaries, loss, accuracy = sess.run([global_step,model.scores, dev_summary_op, model.loss, model.accuracy], feed_dict)
...
classes =['Cell.membrane', 'Cytoplasm', 'Endoplasmic.reticulum', 'Extracellular',"Golgi.apparatus","Lysosome/Vacuole",                  
