...
class CNN_LSTM(object):
    def __init__(self, sequence_length, num_classes, embedding_size, filter_sizes, num_filters, num_hidden):
        ...
        self.tcn = TemporalConvNet(num_chans, stride=1, kernel_size=2, dropout=0.4)
...
#        conv2= bn(conv2, self.training)
        ...
        y = self.tcn(h)
        self.cell_fw = tf.nn.rnn_cell.BasicLSTMCell(64,forget_bias=1.0)
        self.cell_bw = tf.nn.rnn_cell.BasicLSTMCell(64,forget_bias=1.0)        
        self.lstm_out,_ = tf.nn.bidirectional_dynamic_rnn(self.cell_fw,self.cell_bw,y,sequence_length=length(self.input_x),dtype=tf.float32)
        #self.lstm_out,_ = tf.nn.bidirectional_dynamic_rnn(self.cell_fw,self.cell_bw,self.input_x_,dtype=tf.float32)
        self.semantic=tf.concat(self.lstm_out, 2)        
#        print(self.semantic)
...
#            print(y)
        ...
        with tf.name_scope("dropout_hid"):
             ...
             h_drop = tf.nn.dropout(self.semantic, self.dropout_keep_prob)
             print(h_drop)   
        #embed()
        #Attention layer
#        with tf.name_scope('self_attention'):
#            encoder = onmt.encoders.self_attention_encoder.SelfAttentionEncoder(num_layers=1,num_units=128)  
#            self.h_drop, state, encoded_length = encoder.encode(h_drop, sequence_length=length(self.input_x))
#        print(self.h_drop)
        with tf.name_scope('Attention_layer'):
            self.val, alphas = attention(h_drop, num_hidden, return_alphas=True)
