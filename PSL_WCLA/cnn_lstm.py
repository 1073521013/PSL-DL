...
class CNN_LSTM(object):
    def __init__(self, sequence_length, num_classes, embedding_size, filter_sizes, num_filters, num_hidden):
        ...
        val_, state = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, self.h_drop,
                                                      sequence_length=length(self.input_x), dtype=tf.float32)
        ...
        denses = tf.layers.dense(inputs=tf.reshape(drop, shape=[-1, num_hidden * 2]), units=num_hidden,
