...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        if istrain:
            ...
            accuracy, predictions = dev_step(x, y)  # dev_step(x_dev, y_dev, writer=dev_summary_writer)
outputs = np.argmax(predictions, axis=-1)
print(outputs.shape)
confusion_test = ConfusionMatrix(num_classes)
y_train = np.argmax(y, axis=-1)
print(y_train.shape)
confusion_test.batch_add(y_train, outputs)
test_accuracy = confusion_test.accuracy()
cf_test = confusion_test.ret_mat()

from sklearn.metrics import classification_report
print(classification_report(y_train, outputs, labels=None, target_names=None, sample_weight=None, digits=2))
