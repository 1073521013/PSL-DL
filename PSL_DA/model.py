...
from tensorflow.python.ops.rnn_cell import RNNCell, LSTMCell, GRUCell,BasicLSTMCell
...
class BiRNN:
    def __init__(self, num_units, cell_type='lstm', scope=None):
    	self.cell_fw = GRUCell(num_units) if cell_type == 'gru' else BasicLSTMCell(num_units)
        self.cell_bw = GRUCell(num_units) if cell_type == 'gru' else BasicLSTMCell(num_units)
